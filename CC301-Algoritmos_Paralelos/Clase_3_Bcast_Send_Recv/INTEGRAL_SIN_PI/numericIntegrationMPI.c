...
double simpsonRule(double (*f)(double),int a,int b,int n,long int *p)
{
	int i;
...
int main(int argc, char *argv[])
{
  ...
  while(!done) // hasta que todos los procesadores terminen
  {
    ...
    if(n==0)
    {
      done=1; // si es cero, termina todo
      count+=1;
    }
    else
    {
      ...
      count+=1;
      my_result = simpsonRule(Sin,my_a,my_b,interval,&count);
...
//        fprintf(f,"%.18f\t%i\n",endwtime-startwtime,numprocs);
        fprintf(f,"%li\t%.8lf\t%i\n",count,(float)count/(endwtime-startwtime),numprocs);
      	fclose(f);
      }
      else
