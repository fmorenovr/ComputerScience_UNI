...
%%
instruction: %empty                             |
             instruction ID IGUAL expr PUNTCOM  { printf("=> %g\n",$4); } |
             instruction library                |
             ...
             instruction NEWLINE                |
						 instruction binario PUNTCOM        |
						 instruction exprBin          
;
library: LIB '#' ID '#' ;
vars: vars ',' ID | ID
;
exprBin: exprBin MAS binario   |
         exprBin MENOS binario |
				 exprBin MULT binario  |
				 exprBin DIV binario   |
				 binario
;
binario: UNO digitos ;
...
digito: CERO | UNO ;
evalExpr: expr PUNTCOM { printf("=> %g\n",$1); }
;
