...
# PARA CLIENTE SERVIDOR POISSON
# FUNCION DE CLIENTE SERVIDOR CON POISSON-------------
...
PoissonClientServer<-function(T,lambda){ # numero de milisegundos a simular
 ...
 j <- -1/lambda*log(runif(1,0,1)) # genera la primera llegada de tiempo
 type<- 0; 
 arrive<- j; 
 exitQ<- 0;
 exitSys<- 0;
 ...
 while(a>0){
  ...
  exitSys1<-insert(exitSys1,exitSys)
   ...
   if(type == 0){
     n <- n + 1
     j <- arrive -1/lambda*log(runif(1,0,1))
     if(j<=T){ 
...
#FUNCION DE POISSON Uselo para comprobar los tiempos por programa de una sola simulacion


#PoissonClientServer(200,0.1)  # Uselo para comprobar los tiempos por programa


# ----------------------------------------------------------------------


# FUNCION QUE EJECUTA LOS "n" veces el poisson

Ejecutar.n.veces.Client.Servidor.Poisson<-function(n){  # este programa ejecuta "n" veces el programa Bernoulli cliente servidor
...
S<-PoissonClientServer(200,0.1) # este programa nos retorna los tiempos del sistema
...
# Por conveniencia : almacenamos las 100 simulaciones en una variable
 ...
 Datos.tiempo <- Ejecutar.n.veces.Client.Servidor.Poisson(100)
