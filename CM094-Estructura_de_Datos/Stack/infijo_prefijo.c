  ...
  while(j<num)
  {
    ...
    if(esNumero(infijo[j]) ||esLetra(infijo[j])|| isOperator(infijo[j]))
    {
      ...
      if(j<=num-2)
      {
        ...
        {
          if(infijo[j+1]=='^' || infijo[j+1]=='-' || infijo[j+1]=='+' || infijo[j+1]=='(' || infijo[j+1]==')' || infijo[j+1]=='/' || infijo[j+1]=='*' || esNumero(infijo[j+1])||infijo[j+1]=='['||infijo[j+1]==']'||infijo[j+1]=='{'||infijo[j+1]=='}')
            correcto = 1;
        ...
        if(esLetra(infijo[j]))
        {
          if(infijo[j+1]=='^' || infijo[j+1]=='-' || infijo[j+1]=='+' || infijo[j+1]=='(' || infijo[j+1]==')' || infijo[j+1]=='/' || infijo[j+1]=='*' || esLetra(infijo[j+1])||infijo[j+1]=='['||infijo[j+1]==']'||infijo[j+1]=='{'||infijo[j+1]=='}')
            correcto = 1;
          else{ correcto=0;break;}
        } // correcto correcto
...
int cadena_Balanceada()
{
  ...
  if(verificado){
    ...
    {
      return 1; // cadena balanceada
    }
    else
    {
      return 0; // cadena no balanceada
    }
  }
  else
  {
    return 0; // no es correcta la cadena
  }
}
...
int PN() // reverse polish notation
{
  int i,j=0,balanceo,contador;
  char operador;
  int prioridad=0;
  ...
  if(balanceo)
  {
    printf("\nCadena Correcta, empieza el algoritmo pn: \n");
    while(i>=0)
    {
      if(infijo[i]=='*' || infijo[i]=='+' || infijo[i]=='-' || infijo[i]=='/' || infijo[i]=='^')
      {
        if(esNumero(infijo[i-1])||esLetra(infijo[i-1]))  prefijo[j+contador]=' ';
      ...
      else if(esNumero(infijo[i])||esLetra(infijo[i]))
      {
        prefijo[j+contador]=infijo[i];
      }
...
int Calculo_PN(int len){
  ...
  {
    j=0;
    if(prefijo[i]=='('||prefijo[i]=='['||prefijo[i]=='{'||esLetra(prefijo[i])||prefijo[i]==' '||esNumero(prefijo[i]))
      Pila_push(prefijo[i]);
    // Correcto
    else if(prefijo[i]=='+'||prefijo[i]=='-'||prefijo[i]=='*'||prefijo[i]=='/'||prefijo[i]=='^')
    {
      ...
      numero=Operar(numero3,numero2,prefijo[i]);
      numero3=numero;
      cont=0;cifra=0;
      if(numero<0)
      {
    ...
    else if(prefijo[i]==')'||prefijo[i]==']'||prefijo[i]=='}')
    {
      if(pila.s[pila.tope]==' ')  caracter=Pila_pop();
...
int main()
{
  ...
  len=PN();
//  resultado=Calculo_PN(len);
  printf("La forma pn es: \n");
  int m;
  for(m=len;m>=0;m--)
  {
    printf("%c",prefijo[m]);
  ...
  printf("Longuitud del prefijo es: %i",len);
