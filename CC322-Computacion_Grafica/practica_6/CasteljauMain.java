...
public class CasteljauMain {
  ...
  public void inicializarComponentes() {
    ...
    panel.addMouseListener( new MouseAdapter() {
                @Override public void mousePressed(MouseEvent e) {
                    panel.getGraphics().fillOval(e.getX() - 5, e.getY() - 5, 5, 5);
  ...
  public void dibujarBezier(Graphics g, ArrayList<Point> puntos, int numeroPuntos) {
    ...
    for (int k = 0; k < tamLista-1; k=k+1) {
        Double puntoX1 = new Double(puntos.get(k).getX());
        Double puntoY1 = new Double(puntos.get(k).getY());
        Double puntoX2 = new Double(puntos.get(k+1).getX());
        Double puntoY2 = new Double(puntos.get(k+1).getY());
        g.drawLine(puntoX1.intValue(), puntoY1.intValue(), puntoX2.intValue(),puntoY2.intValue());
    }
    //Este ciclo realiza el número de iteraciones que el usuario desee en base 
    //al número de puntos que se desean
    double u=0;
    g.setColor(Color.RED);
    recursividad(u,1,avance,anteriores,tamLista,g);
  ...
  private void recursividad(double a,double limit,double paso,ArrayList<int[]> ant,int lon,Graphics g){
    if(a<limit){
      double puntoX=0,puntoY=0;
      for (int k = 0; k < lon; k++) {
        double b = BersteinPoly(a, lon - 1, k);
        puntoX += puntos.get(k).getX() * b;
        puntoY += puntos.get(k).getY() * b;
      }
      Double d1 = new Double(puntoX);
      Double d2 = new Double(puntoY);
      //Se almacenan y se dibuja el punto calculado anteriormente
      ant.add(new int[] {d1.intValue(), d2.intValue()});           
      g.drawLine(d1.intValue(), d2.intValue(), d1.intValue(),d2.intValue());
      
      //Se dibuja la línea que va del punto anterior al recien calculado
      //de esa manera no tenemos que calcular punto por punto de toda la curva
      if (ant.size() > 1) {
        g.drawLine(ant.get(ant.size() - 2)[0], ant.get(ant.size() - 2)[1],d1.intValue(), d2.intValue());
        //Removemos el primer elemento que ya no se utiliza
        ant.remove(0);
      }
      a=a+paso;
      recursividad(a,limit,paso,ant,lon,g);
    }
    else{ return;}
  }
