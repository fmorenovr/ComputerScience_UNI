...
# ciclo de vida de la celula
  def lifeCycleCell(self):
    self.move()
    nearFood = self.findFoodNearly()
    self.followFood(nearFood)
...
# encuentra la celula con menor distancia
  def findFoodNearly(self):
    foods = getAllFoods()
    miniD = math.hypot(foods[0].x-self.x, foods[0].y-self.y)
    for p in foods:
      d = math.hypot(p.x-self.x, p.y-self.y)
      if d <= miniD:
        miniD = d
        idfoo = p.idfood
    return getIndexFood(idfoo)
